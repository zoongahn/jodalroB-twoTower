...
from preprocess.torchrec.schema import TorchRecSchema, SideSchema


def _check_numerics(name: str, x: torch.Tensor):
    """🔍 전처리 단계에서 NaN/Inf 체크용 헬퍼"""
    if x is None:
        return
    ...
    if has_nan or has_inf:
        raise RuntimeError(
            f"[FeaturePreprocessor] NaN/Inf detected in '{name}'. "
            f"(has_nan={bool(has_nan)}, has_inf={bool(has_inf)})"
        )
        ...
        with torch.no_grad():
            for start_idx in tqdm(
                ...
                if np_numeric is not None:
                    ...
                    # 🔍 원시 numeric 체크
                    _check_numerics(
                        f"{tower_name}.batch_numeric[{start_idx}:{end_idx}]",
                        batch_numeric
                    )
                ...
                if batch_numeric is not None and batch_text:
                    ...
                    _check_numerics(
                        f"{tower_name}.proj_numeric[{start_idx}:{end_idx}]",
                        proj_numeric
                    )
                ...
                # 🔍 projection 결과 전체 체크
                _check_numerics(
                    f"{tower_name}.batch_projected[{start_idx}:{end_idx}]",
                    batch_projected
                )
