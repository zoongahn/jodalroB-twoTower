...
class QueryHelper:
    ...
    def __init__(self, db_connector: DatabaseConnector, source_schema: Optional[str] = None):
        ...
        db_schema = source_schema if source_schema else os.getenv("DB_SCHEMA", "public")
        notice_table = os.getenv("NOTICE_TABLE", "notice")
        company_table = os.getenv("COMPANY_TABLE", "company")
        # bid 테이블은 원본 데이터 테이블 (전처리 전)
        bid_table = os.getenv("BID_TABLE", "bid")

        # 테이블명 정의 (스키마.테이블 형식)
        self.table_names = {
            'notice': f'{db_schema}.{notice_table}',     # 공고 테이블명
            'bid': f'{db_schema}.{bid_table}',           # 투찰 테이블명 (원본)
            'company': f'{db_schema}.{company_table}'    # 업체 테이블명
        }

        self.source_schema = db_schema
    ...
    def get_recent_notices(self, limit: int = None) -> pd.DataFrame:
        """
        최근 공고 목록 조회 (등록일 기준 최신순)

        Args:
            limit: 최대 조회 개수 (None이면 전체 조회)
        """
        if limit is None:
            query = f"""
            SELECT *
